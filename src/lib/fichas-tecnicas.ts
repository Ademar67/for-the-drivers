...
export async function getFichasTecnicas(): Promise<{ items: Ficha[], error?: string, detail?: string }> {
  try {
    ...
    console.error("Error leyendo fichas", err);
    return { error: "Error leyendo fichas", detail: String(err?.message ?? err), items: [] };
  }
}
