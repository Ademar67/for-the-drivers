...
export default function MapaProspectosPage() {
  const mapDivRef = useRef<HTMLDivElement>(null);
  const [map, setMap] = useState<google.maps.Map | null>(null);
  const db = useFirestore();
  const { user, loading: authLoading } = useAuth();

  const [prospectos, setProspectos] = useState<Punto[]>([]);
  ...
  // FIRESTORE: escucha prospectos (solo los del user)
  useEffect(() => {
      ...
      const data: Punto[] = snapshot.docs
        .map((ds) => {
          ...
          return {
            ...
            tipo: (d.tipo ?? "prospecto") as TipoCliente,
            lat: Number(d.lat),
            lng: Number(d.lng),
            diaVisita: dia,
          } as Punto;
  ...
  useEffect(() => {
    ...
    prospectos.forEach((c) => {
      ...
      const marker = new google.maps.Marker({
        ...
        title: c.nombre,
        icon: `http://maps.google.com/mapfiles/ms/icons/${iconColorPorTipo(
          c.tipo
        )}-dot.png`,
      });
      ...
      marker.addListener("click", () => {
        infowindow.setContent(
          `<div style="font-size:14px;font-weight:600">${c.nombre}</div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               <div style="font-size:12px;color:#555">Tipo: ${
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 c.tipo
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               }</div>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          <div style="font-size:12px;color:#555">Día: ${
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            c.diaVisita ||
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            "Sin asignar"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          }</div>`
        );
    ...
    if (markersRef.current.size > 0) map.fitBounds(bounds);
  }, [map, prospectos]);

  // RUTAS
  const seleccionarRutaDeHoy = () => {
    if (!hoy) return;
    const listaHoy = clientesPorDia[hoy] || [];
    if (!listaHoy.length) {
      alert("No hay prospectos asignados para HOY");
      return;
    }
    setRutaSeleccionada(listaHoy);
    setTimeout(() => {
      trazarRuta();
    }, 0);
  };
  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] w-full">
      <div className="w-[420px] overflow-y-auto border-r bg-white p-4">
              ...
              <div key={dia}>
                <div
                  className={`mb-2 text-sm font-semibold ${
                    esHoy ? "text-green-600" : "text-gray-700"
                  }`}
                >
                  {esHoy ? `👉 ${dia} (HOY)` : dia}
                </div>
        ...
        <button
          onClick={seleccionarRutaDeHoy}
          disabled={!hoy || !clientesPorDia[hoy]?.length}
          className="mt-4 w-full rounded bg-emerald-600 py-2 text-white disabled:opacity-50"
        >
          Ruta de HOY
        </button>
