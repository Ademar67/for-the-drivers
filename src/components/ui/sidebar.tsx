    ...
    return (
      ...
      >
        ...
        >
          <div
            data-sidebar="sidebar"
            className="flex h-full w-full flex-col border-r border-sidebar-border/70 bg-sidebar/95 shadow-sm backdrop-blur-sm group-data-[variant=floating]:rounded-2xl group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow-lg"
          >
            {children}
          </div>
        </div>
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "h-9 w-9 rounded-xl border border-border/60 bg-background shadow-sm hover:bg-accent",
        className
      )}
      onClick={(event) => {
        onClick?.(event);
...
const SidebarGroupAction = React.forwardRef<
  HTMLButtonElement,
  Omit<React.ComponentProps<"button">, "ref"> & { asChild?: boolean }
>(({ className, asChild = false, ...props }, ref) => {
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex h-8 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-lg px-2.5 text-sidebar-foreground outline-none ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground",
